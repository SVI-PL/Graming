	...
	public function get_user_products($order_id = '', $user_email = '')
	{
		...
		if (is_user_logged_in()) {
			...
			while ($loop->have_posts()):
				...
				$bought = wc_customer_bought_product_custom($current_user->user_email, $current_user->ID, get_the_ID());
		...
		<div class="wss-ticket-container" style="display: none;">
			<form method="POST" class="create-new-ticket" action="">
				<div class="form_input form_select">
					...
					<select name="product-id" class="product-id">
						<?php
				...
				<?php wp_editor('', 'wss-ticket', array(
					'wpautop' => 0,
					'media_buttons' => 0,
					'textarea_name' => 'wss-ticket',
					'textarea_rows' => 20,
					'tabindex' => null,
					'editor_css' => '',
					'editor_class' => '',
				...
				<div class="btn-red back-to-tickets">Cancel</div>
				<input type="submit" class="send-new-ticket" value="<?php echo __('Send', 'wc-support-system'); ?>">
			</form>
			<div class="bootstrap-iso product-alert"></div>
		</div>
		<a class="button new-ticket">
			<?php echo __('New ticket', 'wc-support-system'); ?>
		</a>
		<a class="button ticket-cancel" style="display: none;">
		...
		if ($this->logged_in_user_support_access_validation()):
			...
			if ($tickets) {
				...
				<table class="table support-tickets-table">
					<thead>
						<th class="id" style="padding: 0.5em 1.5rem;">
							<?php echo __('ID', 'wc-support-system'); ?>
						</th>
						<th class="subject">
							<?php echo __('Subject', 'wc-support-system'); ?>
						</th>
						<th class="create-time">
							<?php echo __('Creation time', 'wc-support-system'); ?>
						</th>
						<th class="update-time">
							<?php echo __('Update time', 'wc-support-system'); ?>
						</th>
						<th>
							<?php echo __('Product', 'wc-support-system'); ?>
						</th>
						<th>
							<?php echo __('Status', 'wc-support-system'); ?>
						</th>
						<th>
							<?php echo "Action"; ?>
						</th>
					...
					<tbody>
						...
						foreach ($tickets as $ticket) {
							...
							echo '<td class="subject">' . stripcslashes($ticket->title) . '</td>';
							...
							/*Product image*/
							$thumbnail = get_the_title($ticket->product_id);
							if ($thumbnail) {
								$image = $thumbnail;
							}
							...
							echo '<td class="action ticket-toggle" data-ticket-id="' . $ticket->id . '"><span class="btn-action">Details</span></td>';
							echo '</tr>';
						}
						?>
					</tbody>
						...
						<div class="modal-content">
							<div class="modal-header">
								...
								<h4 class="modal-title">
									<?php echo __('Change the ticket status', 'wc-support-system'); ?><span></span>
								</h4>
							</div>
							<div class="modal-body">
								<div class="row status-selector">
									<div class="col-xs-4 status status-1" data-status="1">
										<?php echo self::get_ticket_status_label(1); ?>
									</div>
									<div class="col-xs-4 status status-2" data-status="2">
										<?php echo self::get_ticket_status_label(2); ?>
									</div>
									<div class="col-xs-4 status status-3" data-status="3">
										<?php echo self::get_ticket_status_label(3); ?>
									</div>
								</div>
							</div>
							<div class="modal-footer">
								<button type="button" class="btn btn-default" data-dismiss="modal">
									<?php echo __('Close', 'wc-support-system'); ?>
								</button>
							</div>
						</div>
					</div>
