...
class WECMF_Settings {
	...
	public function __construct() {
		...
		add_action('wp_ajax_hide_thwecmf_admin_notice', array($this, 'hide_thwecmf_admin_notice'));
		...
		add_action( 'admin_init', array( $this, 'wecmf_notice_actions' ), 20 );
		...
		add_filter('woocommerce_email_styles', array($this, 'rewrite_default_woo_email_styles'), 10, 2);
		...
		add_action('admin_footer-plugins.php', array($this, 'wecmf_deactivation_form'));
		add_action('wp_ajax_thwecm_deactivation_reason', array($this, 'thwecm_deactivation_reason'));
	}
	...
	public function init() {
		...
		add_filter('wc_get_template', array($this, 'thwecmf_woo_get_template'), 10, 5);
	...
	public function thwecmf_woo_get_template( $template, $template_name, $args, $template_path, $default_path ){
		if( $this->thwecmf_skip_template($template_name) ){
			return $template;
		}
	...
	public function enqueue_admin_scripts($hook){
		...
		wp_enqueue_script( 'thwecmf-admin-misc', plugins_url('/assets/misc_js/thwecmf-admin-misc.min.js', dirname(__FILE__)), ['jquery'], TH_WECMF_VERSION, true );
	...
	public function output_review_request_link(){
		if(!apply_filters('thwecmf_show_dismissable_admin_notice', true)){
			return;
		}
		...
		$now = time();
		$dismiss_life  = apply_filters('thwecmf_dismissed_review_request_notice_lifespan', 6 * MONTH_IN_SECONDS);
		$reminder_life = apply_filters('thwecmf_skip_review_request_notice_lifespan', 7 * DAY_IN_SECONDS);
		
		$is_dismissed   = get_user_meta( get_current_user_id(), 'thwecmf_review_dismissed', true );
		$dismisal_time  = get_user_meta( get_current_user_id(), 'thwecmf_review_dismissed_time', true );
		...
		$is_skipped = get_user_meta( get_current_user_id(), 'thwecmf_review_skipped', true );
		$skipping_time = get_user_meta( get_current_user_id(), 'thwecmf_review_skipped_time', true );
		...
		$thwecmf_since = get_option('thwecmf_since');
		if(!$thwecmf_since){
			$now = time();
			update_option('thwecmf_since', $now, 'no' );
		}

		$thwecmf_since = $thwecmf_since ? $thwecmf_since : $now;
		$render_time = apply_filters('thwecmf_show_review_banner_render_time' , 7 * DAY_IN_SECONDS);
		$render_time = $thwecmf_since + $render_time;
		if($now > $render_time ){
	...
	public function review_banner_custom_css(){
		...
		?>
        <style>
            .thwecmf-review-wrapper {
			    padding: 15px 28px 26px 10px !important;
			    margin-top: 35px;
			}
			.thwecmf-review-image {
			    float: left;
			}
			.thwecmf-review-content {
			    padding-right: 180px;
			}
			.thwecmf-review-content p {
			    padding-bottom: 14px;
			}
			.thwecmf-notice-action{ 
			...
			.thwecmf-notice-action:hover:not(.thwecmf-yes) {
			    background-color: #f2f5f6;
			}
			.thwecmf-notice-action.thwecmf-yes:hover {
			    opacity: .9;
			}
			.thwecmf-notice-action .dashicons{
			    display: none;
			}
			.thwecmf-themehigh-logo {
			    position: absolute;
			    right: 20px;
			    top: calc(50% - 13px);
			}
	...
	private function render_review_request_notice(){
		...
		$dismiss_url = add_query_arg(array('thwecmf_dissmis' => true, 'thwecmf_review_nonce' => wp_create_nonce( 'thwecmf_notice_security')));
		...
		<div class="notice notice-info thpladmin-notice is-dismissible thwecmf-review-wrapper" data-nonce="<?php echo wp_create_nonce( 'thwecmf_notice_security'); ?>">
			...
			<div class="thwecmf-themehigh-logo">
				<span class="logo" style="float: right">
            		<a target="_blank" href="https://www.themehigh.com">
                		<img src="<?php echo esc_url(TH_WECMF_URL .'assets/images/themehigh-logo.png'); ?>" style="height:19px;margin-top:4px;" alt="themehigh"/>
                	</a>
                </span>
			</div>
	    </div>
	...
	public function wecmf_notice_actions(){
		...
		$thwecmf_remind = isset($_GET['thwecmf_remind']) ? sanitize_text_field( wp_unslash($_GET['thwecmf_remind'])) : false;
		if($thwecmf_remind){
			update_user_meta( get_current_user_id(), 'thwecmf_review_skipped', true );
			update_user_meta( get_current_user_id(), 'thwecmf_review_skipped_time', $now );
		}

		$thwecmf_dissmis = isset($_GET['thwecmf_dissmis']) ? sanitize_text_field( wp_unslash($_GET['thwecmf_dissmis'])) : false;
		if($thwecmf_dissmis){
			update_user_meta( get_current_user_id(), 'thwecmf_review_dismissed', true );
			update_user_meta( get_current_user_id(), 'thwecmf_review_dismissed_time', $now );
		}

		$thwecmf_reviewed = isset($_GET['thwecmf_reviewed']) ? sanitize_text_field( wp_unslash($_GET['thwecmf_reviewed'])) : false;
		if($thwecmf_reviewed){
			update_user_meta( get_current_user_id(), 'thwecmf_reviewed', true );
			update_user_meta( get_current_user_id(), 'thwecmf_reviewed_time', $now );
		}
	}

	public function hide_thwecmf_admin_notice(){
		check_ajax_referer('thwecmf_notice_security', 'thwecmf_review_nonce');
	...
	public function get_admin_url($page = false){
		$url = 'admin.php?';
		if($page && !empty($page)){
			$url .= '&page='. $page;
		}
	...
	private function thwecmf_skip_template($template_name){
		$skip = false;
		if( $template_name === "emails/email-order-details.php" ){
			$skip = true;
		}else if( $template_name === "emails/email-downloads.php" ){
			$skip = true;
		}
		return apply_filters('thwecmf_skip_template', $skip);
	}

	public function rewrite_default_woo_email_styles($styles, $email){
		$id = isset($email->id) ? $email->id : false;
		if($id){
			$key = str_replace('_', '-', $id);
			$key = in_array($id, $this->admin_emails) ? 'admin-'.$key : $key;
			if( is_array( $this->template_map ) && isset($this->template_map[$key]) && $this->template_map[$key] != '' ){
				if(file_exists(TH_WECMF_PATH.'classes/inc/email-styles.php')){
					ob_start();
					include TH_WECMF_PATH.'classes/inc/email-styles.php';
					return ob_get_clean();
				}
			}
		}
		...
		public function wecmf_deactivation_form(){
		$is_snooze_time = get_user_meta( get_current_user_id(), 'thwecmf_deactivation_snooze', true );
		$now = time();
		...
		?>
		<div id="thwecmf_deactivation_form" class="thpladmin-modal-mask">
			<div class="thpladmin-modal">
				<div class="modal-container">
					<!-- <span class="modal-close" onclick="thwecmfCloseModal(this)">×</span> -->
		...
		<style type="text/css">
			...
			.thpladmin-modal .thwecm-privacy-cnt {
			    color: #919191;
			    font-size: 12px;
			    margin-bottom: 31px;
			    margin-top: 18px;
			...
			.thwecm-right-link.thwecm-active {
			    background: #0773AC;
			    color: #fff;
			}
			.thwecm-title-text {
			...
			.get-support-version-b .thwecm-right-link {
			    background-image: url(<?php echo esc_url(TH_WECMF_URL .'assets/images/get_support_icon.svg'); ?>);
			    background-repeat: no-repeat;
			...
			span.wecm-rating-link {
			    color: #ffb900;
			}
			.thwecm-review-and-deactivate {
			    text-decoration: none;
			}
		</style>
		...
		<script type="text/javascript">
			(function($){
				var popup = $("#thwecmf_deactivation_form");
				var deactivation_link = '';

				$('.thwecmf-deactivate-link').on('click', function(e){
					e.preventDefault();
					deactivation_link = $(this).attr('href');
					popup.css("display", "block");
					popup.find('a.thwecmf-deactivate').attr('href', deactivation_link);
				});
				...
				popup.on('click', '.thwecm-close', function () {
					popup.css("display", "none");
				});

				popup.on('click', '.thwecm-submit-deactivate', function (e) {
                    e.preventDefault();
                    var button = $(this);
                    ...
                    var wecm_deacive_nonce = parent_ul.data('nonce');

                    $.ajax({
                        url: ajaxurl,
                        type: 'POST',
                        data: {
                            action: 'thwecm_deactivation_reason',
                            reason: (0 === radio.length) ? 'none' : radio.val(),
                            comments: (0 !== input.length) ? input.val().trim() : '',
                            security: wecm_deacive_nonce,
                        },
                        beforeSend: function () {
	...
	public function thwecm_deactivation_reason(){
		global $wpdb;

		check_ajax_referer('thwecm_deactivate_nonce', 'security');
		...
		if($_POST['reason'] === 'temporary'){
			...
			update_user_meta(get_current_user_id(), 'thwecmf_deactivation_snooze', $snooze_time);
		...
		$data = array(
	        ...
	        'plugin_version'=> TH_WECMF_VERSION
		);
	...
	public function quick_links(){
		...
		<div class="th_quick_widget-float">
			<div id="myDIV" class="th_quick_widget">
				<div class="th_whead">
					<div class="th_whead_close_btn" onclick="thwecmfwidgetClose()">
						<svg
						width="8"
                		height="8"
                		viewBox="0 0 8 8"
        			...
        			<div class="th_quick_widget_body">
        				<ul>
        					<li>
                    			...
                    			<a href="https://app.loopedin.io/email-customizer-for-woocommerce" target="_blank" class="quick-widget-doc-link">Request a feature</a></li>
               			 	<li>
                    			...
                    			<a href="https://www.themehigh.com/product/woocommerce-email-customizer/?utm_source=free&utm_medium=quicklinks&utm_campaign=wec_upgrade_link" target="_blank" class="quick-widget-doc-link">Upgrade to Premium</a></li>
               			 	<li>
				          	...
				          	<li>
		              			...
		              			</div><a href="https://wordpress.org/support/plugin/email-customizer-for-woocommerce/" target="_blank" class="quick-widget-support-link">Get support</a></li>
		              		<!--<li>
			...
			<div id="myWidget" class="widget-popup" onclick="thwecmfwidgetPopUp()">
