...
function graming_setup() {
	/*
		...
		* If you're building a theme based on Graming, use a find and replace
		* to change 'graming' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'graming', get_template_directory() . '/languages' );
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'graming_custom_background_args',
			array(
 ...
 */
function graming_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'graming_content_width', 640 );
}
add_action( 'after_setup_theme', 'graming_content_width', 0 );
...
function graming_scripts() {
	...
	wp_style_add_data( 'graming-style', 'rtl', 'replace' );
